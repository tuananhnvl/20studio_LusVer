...
void main()
{
    vec2 u_resolution1 = vec2(200.,200.);
    vec2 uvN = (gl_FragCoord.xy - u_resolution1.xy*0.5)/u_resolution1.y;
   // vec2 uv = gl_FragCoord.xy;
    t=u_time*10.;
    vec3 from=path(t);
    adv=path(t+6.+sin(t*.1)*3.);
    vec3 dir=normalize(vec3(uvN,0.1));
    dir=lookat(adv-from,vec3(0.,.5,0.))*dir;
    vec3 col=march(from, dir);
 
    gl_FragColor=vec4(col,1.);
   // gl_FragColor=vec4(vec3(1.0,0.2,0.9),1.);
}
`
const spaceFragment = `
void main()
