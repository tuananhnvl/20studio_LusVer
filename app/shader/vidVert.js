...
vec3 getBasePosition(in vec3 pos,in vec2 domWH){vec3 basePos=vec3((pos.xy)*domWH-u_domPivot,pos.z);
...
float cubicBezier(float p0,float p1,float p2,float p3,float t){
    float c=(p1-p0)*3.;float b=(p2-p1)*3.-c;float a=p3-p0-c-b;
    float t2=t*t;
    float t3=t2*t;
...
vec3 deformationCurve(vec3 position, vec2 uv, float wave,float centerPoint,float uScroll) {
    float M_PI = 3.1415926535897932384626433832795;
    position.x = position.x + (sin(uv.y * M_PI) );
    position.y = position.y + (sin(uv.x * M_PI));
    ...
    void main(){
        ...
        domXY.x+=mix(domWH.x,0.,cos(v_showRatio*PI*2.)*0.5+0.5)*0.1;
        ...
        float angle = (u_time) * 3.14159265 / 2.;
        float wave = cos(angle); 
        
        float center = cos(length(uv - .5) * 2.101235);

       vec3 newPosition = deformationCurve(position,uv,wave,center,(u_time/1000.));
